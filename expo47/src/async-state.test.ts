...
test("useAsyncEffect lifecycle", async () => {
    ...
    await act(() => state.status === 'pending' ? state.promise : fail("state is not pending"))
...
test.only("useAsyncEffect simple failure", async () => {
    let state: RemoteData<void> = { status: 'init' }
    renderHook(() => {
        state = useAsyncEffect(async () => { throw new Error('oops') })
    })
    expect(state.status).toBe('pending')
    await act(() => state.status === 'pending' ? state.promise : fail("state is not pending"))
...
test.only("useAsyncState simple failure", async () => {
    let state: RemoteData<number> = { status: 'init' }
    renderHook(() => {
        state = useAsyncState(async () => { throw new Error('oops') })
    })
    expect(state.status).toBe('pending')
    await act(() => state.status === 'pending' ? state.promise : fail("state is not pending"))
    expect(state.status).toBe('failure')
