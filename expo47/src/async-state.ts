...
export function map<V, T>(d: RemoteData<V> | Result<V>, fn: (v: V) => T): RemoteData<T> {
    return isSuccess(d)
        ? { status: 'success', value: fn(d.value) }
        : d
}
export function fold<V, T>(d: RemoteData<V>, initFn: () => T, pendingFn: (p: Promise<void>) => T, failureFn: (e: any) => T, successFn: (v: V) => T): T
export function fold<V, T>(d: Result<V>, failureFn: (e: any) => T, successFn: (v: V) => T): T
export function fold<V, T>(d: any, arg1: any, arg2: any, arg3?: any, arg4?: any): T {
    if (arg3) {
        // remotedata
        switch (d.status) {
            case "init": return arg1()
            case "pending": return arg2(d.promise)
            case "failure": return arg3(d.error)
            case "success": return arg4(d.value)
            default: throw new Error(`bad RemoteData: ${d}`)
        }
    }
    else {
        // result
        switch (d.status) {
            case "failure": return arg1(d.error)
            case "success": return arg2(d.value)
            default: throw new Error(`bad Result: ${d}`)
        }
    }
}
 ...
 */
export function useAsyncState<V>(effect: AsyncState<V>, deps?: React.DependencyList): RemoteData<V>
export function useAsyncState<V>(opts: AsyncStateOpts<V>): RemoteData<V>
export function useAsyncState<V>(arg1: AsyncStateOpts<V> | AsyncState<V>, arg2?: React.DependencyList): RemoteData<V> {
    const opts: AsyncStateOpts<V> = typeof arg1 === 'function'
        ? { effect: arg1, deps: arg2 }
        : arg1
 ...
 */
export function useAsyncEffect(effect: AsyncEffect, deps?: React.DependencyList): AsyncEffectResult
export function useAsyncEffect(opts: AsyncEffectOpts): AsyncEffectResult
export function useAsyncEffect(arg1: AsyncEffectOpts | AsyncEffect, arg2?: React.DependencyList): AsyncEffectResult {
